...
My thinking about reasoning was the first piece to become clear. Reading about deliberative alignment gave me a strong push. The idea of extending chain-of-thought to improve safety made sense, but I felt it used reasoning in a restricted way. Reasoning, to me, is synonymous with generalization, and generalization is one of the few capabilities that naturally scales with model size. This led me to wonder whether reasoning could be redirected toward something more fundamental. Instead of matching rules, could a model use its reasoning to anticipate the downstream events implied by its own potential outputs. If a model can project forward in a task domain, it should also be able to project forward in value space. That belief eventually became my interest in what I call linguistic “world simulation,” the attempt to let a model judge the consequences of its own answers even when everything happens in text. My recent paper is a first attempt at this idea. The results are not dramatic, which is probably expected given how limited language-only simulation is, but the exploration clarified the direction. It showed me that reasoning alone cannot carry the burden unless the model has something meaningful to reason about.
