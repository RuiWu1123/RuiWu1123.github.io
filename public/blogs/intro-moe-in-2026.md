...
Everything so far has assumed learned routing: a trainable gate scores experts, and gradient descent shapes those scores over time. That's the industry default, but not the only option — here are the other two.
...
The router's core job is turning a token's representation into a score per expert, and which function does that scoring is one of the key design choices here.
...
The shared-expert question is the opposite of converged. DeepSeek-V3, GLM-5.2, and Kimi K3 keep one; MAI-Thinking-1 tested one and dropped it; Qwen added shared experts for over a year, removed them entirely in Qwen3. If there's a single design axis in this whole survey where the field visibly hasn't made up its mind, this is it.
