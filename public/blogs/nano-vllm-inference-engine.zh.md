...
拿第 3 轮结束时算一遍：A 的 prompt 是 5 个 token，占 ⌈5/4⌉ = 2 块；B 同样 2 块；C 的 prompt 是 7 个，也是 2 块。加起来正好 6 块，池子被分光了。这时候实际存着的 token 只有 17 个，池子名义上能装 24 个，可剩下的 7 个槽位分散在三个序列各自没填满的尾块里，互相借不到。这份"看着还有空、实际拿不出来"的浪费，就是 block 大小带来的内部碎片，也正是下一节 PagedAttention 要把它压到一个 block 以内的东西。

图里的格子这么读。prefill 格子里的数字是这一轮吃掉的 prompt token 数，所以是 5 或 7；decode 格子恒定是 1，因为一轮只生成一个 token。还有一点图上看不出来但值得知道：prefill 那一轮除了消化 prompt，末尾也会顺手产出第一个生成 token，所以标着 gen 8 的 A 只需要再跑 7 个 decode 轮。灰色带横杠的格子表示这条序列在跑，但这一轮什么也没做。
...
两张图最扎眼的差别在 C 这一行。静态那张里，C 第 3 轮就到了，却一直排到第 10 轮才轮上，中间七轮什么也没干。这期间 B 早在第 4 轮就结束，显存也空了出来，可 C 依然进不来，因为它不属于当前这一批。continuous 那张里，C 第 3 轮就被放进来，之后它的 decode 和 A 的 decode 挤在同一轮里并行推进。同样的工作量，continuous 跑完用了 11 轮，静态用了 15 轮。

C 那一行还有第二处值得看：第 5 轮它被抢占了。那一轮 C 已经攒到 9 个 token，`9 % 4 == 1` 意味着它刚好迈进第 3 个 block，必须再拿一块才能继续写。可池子早在第 3 轮就分光了，而这一轮 A 和 B 已经先被排进去、从 running 里取走了，轮到 C 的时候队列里一个可以驱逐的对象都没有。于是它只能抢占自己：block 全部释放，退回等待队列的最前面，刚算完的 prompt 作废。到第 6 轮，B 结束腾出的空间加上 C 自己吐出来的，够它重新 prefill 了。

这正好命中下面那段代码里最不起眼的那个分支，也就是 `else: self.preempt(seq)`，"没有别人可赶，只好赶自己"的那条路。
