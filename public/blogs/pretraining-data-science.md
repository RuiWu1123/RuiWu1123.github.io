...
$R_D^{*}$ is therefore not an independent quantity with a meaning of its own. It is the per-repeat decay rate $\delta$ in different clothes, chosen because $(1-\delta)/\delta$ reads as a half-life. The fitted $\delta \approx 6\times 10^{-2}$ is what gives $R_D^{*} = 15.39$. Effective parameters get the same treatment, with $U_N = \min\{N_{\mathrm{opt}}, N\}$ and $R_N = N/U_N - 1$.
