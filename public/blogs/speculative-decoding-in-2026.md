...
The acceptance probability `min(1, p[x]/q[x])` is precisely what saturates that bound. A token is proposed with probability `q(x)` and kept with probability `min(1, p(x)/q(x))`, so it survives with probability `q(x)·min(1, p(x)/q(x)) = min(p(x), q(x))`. That is the ceiling, hit at every token at once.

Which then forces everything else. Having emitted `min(p, q)`, what is still owed to `p` is `p − min(p, q)`, and that equals `(p − q)⁺`, meaning the elementwise `max(p − q, 0)`. The truncation is not cosmetic. At tokens where `p < q` the accept path already delivered `min(p,q) = p`, the full amount `p` asks for, so the shortfall there is zero rather than negative; the deficit lives entirely at tokens the drafter under-weighted. And its total size is not free either: `Σ(p − q)⁺ = 1 − α`, exactly the probability of rejecting. So drawing rejections from `(p − q)⁺` renormalized makes the two paths sum to `min(p,q) + (1−α)·(p−q)⁺/(1−α) = p`. There is nothing left to tune.

Two ways of getting this wrong are worth naming, because both are tempting. Normalizing `p − q` without the truncation is not merely worse, it is undefined: `Σ(p − q) = 0`, so you would be dividing by zero. And resampling from `p` on rejection, which is the obvious guess, double-counts whatever `q` already covered. Take `p = (0.6, 0.4)` against `q = (0.2, 0.8)`. Here α = 0.6, the residual is `(0.4, 0)` which renormalizes to `(1, 0)`, and the two paths return exactly `(0.6, 0.4)`. Resampling from `p` instead returns `(0.44, 0.56)`, which simulation confirms at `(0.4405, 0.5595)`. Visibly wrong on two tokens, and wrong in a way that would be very easy to miss on a vocabulary of 100,000.

Worth saying plainly that none of this originates with speculative decoding. Sharing `min(p, q)` between two distributions is the *maximal coupling*, a classical construction, and `1 − TV` as the best achievable agreement is its standard result. What the 2023 papers contributed is the application, together with the one-shot structure: classic rejection sampling redraws from `q` until something is accepted, with no bound on the number of tries, whereas the coupling gives one attempt and a residual to fall back on, so every position terminates after exactly one draw.
...
One consequence deserves stating, since it explains why α matters so much. Draft token k+1 was produced conditioned on token k being `x`, and the target's own distribution at position k+1, computed in that same pass, is conditioned on the same thing. So a rejection at position k does not cost you one token: every later draft, and every later distribution the pass already computed, refers to a prefix that no longer exists. One rejection discards the entire remainder of the round, which is why the yield is geometric rather than linear.

A round drafts γ tokens and verifies them in one target pass. If the first rejection lands at position k you emit k+1 tokens, the k accepted ones plus the resampled replacement; if all γ survive you emit γ+1, the extra being a free sample from the target's own distribution at the final position, which the verification pass already computed. Under the paper's stated assumption that per-token acceptance is i.i.d. with rate α, the yield of a round is a capped geometric, `(1 − α^(γ+1))/(1 − α)`, which holds to within sampling error at every α and γ I tried. Drafting four at a time at α = 0.75, twenty-one tokens took six target passes instead of twenty-one:
...
The yield of a round is `1 + Σ_k Π_{i≤k} α_i`, which is the general form of section 1's capped geometric: set every α_i equal and it collapses back to `(1 − α^(γ+1))/(1 − α)`. Because it is a product, early positions are weighted enormously more than late ones, and a decaying tail costs more than the average acceptance suggests. In the three profiles above, the decaying one averages a respectable acceptance across the block but yields 3.81 tokens per round against 4.33 for the flat one.
