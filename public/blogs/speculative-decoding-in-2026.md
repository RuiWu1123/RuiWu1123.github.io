...
There are two natural ways to write that resampling step, and both are wrong, each for a different reason. Normalizing `p − q` without the truncation is not merely worse, it is undefined: `Σ(p − q) = 0`, so you would be dividing by zero. And resampling from `p` on rejection, which is the obvious guess, double-counts whatever `q` already covered. Take `p = (0.6, 0.4)` against `q = (0.2, 0.8)`. Here α = 0.6, the residual is `(0.4, 0)` which renormalizes to `(1, 0)`, and the two paths return exactly `(0.6, 0.4)`. Resampling from `p` instead returns `(0.44, 0.56)`, which simulation confirms at `(0.4405, 0.5595)`. Visibly wrong on two tokens, and wrong in a way that would be very easy to miss on a vocabulary of 100,000.
...
The guarantee is directly measurable. I took a target `p` over eight tokens, deliberately picked a bad draft `q` sitting 0.34 in total variation away from it, and ran two million single-token speculative steps.
