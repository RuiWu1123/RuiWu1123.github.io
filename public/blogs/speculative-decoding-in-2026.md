...
[EAGLE-3](https://arxiv.org/abs/2503.01840) fixes it with what the authors call a training-time test: during training, simulate the multi-step drafting process, so the head is trained on the distribution it will actually face. The same paper makes a second change, dropping feature prediction in favor of predicting tokens directly, and reports that the combination lets the drafter keep improving with more training data, which the feature-prediction formulation did not.
